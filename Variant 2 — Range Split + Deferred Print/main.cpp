...
int main(){
    ...
    mutex vec_mx; vector<uint32_t> primes; vector<thread> th;
    ...
    for(int t=0;t<cfg.threads;t++){
        ...
        th.emplace_back([&,s,e]{vector<uint32_t>local;
            for(uint64_t i=s;i<e;i++){uint32_t n=3+i*2;if(is_prime(n))local.push_back(n);}
            lock_guard<mutex>l(vec_mx);primes.insert(primes.end(),local.begin(),local.end());
        });
    }
    for(auto&x:th)x.join();
    sort(primes.begin(),primes.end());
    for(auto n:primes) cout<<"["<<now_ts()<<"] prime="<<n<<"\n";
    cout<<"["<<now_ts()<<"] RUN END\n";
}